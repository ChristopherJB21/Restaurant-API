...
func NewUserService(userRepository repository.IUserRepository, validate *validator.Validate, rSAPrivateKey *rsa.PrivateKey) IUserService {
	return &UserService{
...
func (service *UserService) Login(ctx context.Context, request model.UserLoginRequest) model.UserLoginResponse {
	...
	if match {
		token, err := helper.GenerateToken(findUser, service.RSAPrivateKey)
		helper.PanicIfError(err)
